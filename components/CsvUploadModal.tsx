    ...
    reader.onload = (e) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          // Clean only quotes, DO NOT trim
          const cleanedValues = rowData.map(val => val.replace(/^"|"$/g, ''));

          if (cleanedValues.length < headers.length) continue;

          const item: any = { usage: [] };

          headers.forEach((header, index) => {
            const rawValue = cleanedValues[index];       // preserves spaces
            const trimmed = rawValue.trim();             // trimmed version for other fields

            switch (header) {
              case 'project id': item.projectId = trimmed; break;
              case 'pr number': item.prNumber = trimmed; break;
              case 'item code': item.itemCode = trimmed; break;
              case 'description': 
                item.description = rawValue;   // KEEP ALL SPACES EXACTLY AS WRITTEN
                break;
              case 'weight': item.weight = parseFloat(trimmed) || 0; break;
              case 'pr qty': item.prQty = parseFloat(trimmed) || 0; break;
              case 'required qty': item.requiredQty = parseFloat(trimmed) || 0; break;
              case 'received qty': item.receivedQty = parseFloat(trimmed) || 0; break;
            }
          });
    ...
    <div className="w-full max-w-2xl">
      ...
      <div className="mb-6 bg-blue-50 border border-blue-100 rounded-lg p-4">
        ...
        <ul className="list-disc list-inside text-sm text-blue-700 space-y-1">
          ...
          <li className="font-mono text-xs bg-white inline-block px-1 rounded">
            Project ID, PR Number, Item Code, Description, Weight, PR Qty, Required Qty, Received Qty
          </li>
